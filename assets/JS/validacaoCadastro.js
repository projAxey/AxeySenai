...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('iCadastroUsuarios').addEventListener('submit', function (event) {
        ...
        var telefoneValido = telefone.length === 10;
        var emailValido = validarEmail(email);
        var cpf = document.getElementById('cpf').value.replace(/\D/g, '');
        var cnpj = document.getElementById('cnpj').value.replace(/\D/g, '');
        var senha = senhaInput.value;
        var senhaRepetida = senhaRepetidaInput.value;

        if (!celularValido) {
            event.preventDefault();
            alert('Número de celular inválido. Certifique-se de que o número tenha 11 dígitos e comece com 9 após o DDD.');
        }

        if (!telefoneValido) {
            event.preventDefault();
            alert('Número de telefone inválido. Certifique-se de que o número tenha 10 dígitos e comece com 3 após o DDD.');
        }
